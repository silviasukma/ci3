...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        ...
        $config['smtp_user'] = 'silviasukma341@gmail.com';
        $config['smtp_pass'] = 'pdasnimqrxpcvobb';
        $config['smtp_port'] = '465';
        $config['malltype'] = 'html';
        $config['charset'] = 'utf-8';
        $config['newline'] = "\r\n";
        $this->email->initialize($config);
        $this->email->from('silviasukma341@gmail.com', 'Sukma');
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Clik this link to verify : 
                <a href="' . base_url() . 'auth/resetpassword?email=' .
                $this->input->post('email') . '&token=' . urlencode($token) . '">Actived</a>');
        }
        if ($this->email->send()) {
            return true;
        } else {
